...
function activeBackend() {
  if (process.env.RPOW_BACKEND === 'node') return 'node';
  if (process.env.RPOW_BACKEND === 'native') return 'native';
  return native.isAvailable() ? 'native' : 'node';
}

// Mine a single challenge using either the Rust binary (preferred) or Node
// worker_threads. Backend can be forced via $RPOW_BACKEND=native|node.
