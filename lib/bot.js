...
async function fetchLedgerCached() {
  ...
  // Find any working session to make the request. Each attempt is
  // capped at 5s so a stuck endpoint doesn't block /status indefinitely.
  for (const p of session.list()) {
    ...
    try {
      const l = await withTimeout(api.ledger(state), 5000, '/ledger');
...
function describeProfileError(err) {
  if (err instanceof ApiError) {
    ...
    if (err.code === 'TIMEOUT') {
      return 'API slow — try /status again in a moment';
    }
    if (err.code === 'HTTP_504' || err.status === 504) {
      return 'API gateway timeout (server overloaded)';
    }
...
// Race a promise against a timeout. We cap upstream API calls so a hung
// TCP connection (Node's default fetch has no read timeout) can't freeze
// an interactive bot response for 30+ seconds.
function withTimeout(promise, timeoutMs, label = 'request') {
  let timer;
  const timeout = new Promise((_, reject) => {
    timer = setTimeout(
      () => reject(new ApiError('TIMEOUT', `${label} timeout after ${timeoutMs}ms`, 0)),
      timeoutMs,
    );
  });
...
// Single /me fetch with two attempts on different timeout budgets.
// Empirical: api.rpow2.com /me returns either as a fast 502 (5-10s) or a
// slow 200 (18-22s) tonight. A flat 7s timeout missed every slow success.
// We try a short timeout first to catch fast paths quickly, then a long
// one to catch the slow-but-OK case before falling back to cached data.
//   Attempt 1: 10 s -- fast 502 fails fast, fast success returns fast.
//   Attempt 2: 25 s -- catches the 18-22 s slow successes.
// Worst-case wall-clock per profile: ~35 s. Profiles run in parallel
// (Promise.all) so total /status latency is bounded by the slowest
// profile, not the sum.
async function fetchMeWithRetry(state) {
  const timeouts = [10000, 25000];
  let lastErr;
  for (let attempt = 0; attempt < timeouts.length; attempt++) {
    try {
      const me = await withTimeout(api.me(state), timeouts[attempt], '/me');
      return { ok: true, me };
    } catch (e) {
      lastErr = e;
      if (
      ...
      // Brief gap so we don't pile two requests onto a struggling backend
      // back-to-back.
...
async function buildStatusMessage() {
  ...
  // Format an "as of X ago" stale data indicator. We use the bot-state
  // cache timestamp to tell the user how fresh the cached numbers are.
  const fmtStaleAge = (atMs) => {
    const ageMs = now - atMs;
    if (!isFinite(ageMs) || ageMs < 0) return '';
    const minutes = ageMs / 60000;
    if (minutes < 1) return `${Math.max(1, Math.round(ageMs / 1000))}s ago`;
    return fmtDuration(minutes) + ' ago';
  };

  // Start from a copy of the previous profile cache so a failure in this
  // poll never wipes a cached entry from a previous successful poll. We
  // only ever ADD or UPDATE per-profile state below, never delete.
  const newProfilesState = { ...prev.profiles };
  ...
  for (const r of results) {
    ...
    if (r.error) {
      // API failed for this profile. Fall back to last cached numbers so
      // the user never sees "0 RPOW" purely because the network blipped.
      warn += 1;
      lines.push(`👤 <b>${esc(r.name)}</b>`);
      if (r.email) lines.push(`<code>${esc(r.email)}</code>`);
      const cached = prev.profiles[r.name];
      if (cached && typeof cached.balance === 'number' && typeof cached.at === 'number') {
        stale += 1;
        const cBal = Number(cached.balance) || 0;
        const cMin = Number(cached.minted) || 0;
        totalBalance += cBal;
        totalMinted += cMin;
        lines.push(
          `💎 <b>${fmtNum(cBal)}</b> RPOW · ⛏️ ${fmtNum(cMin)} <i>(cached ${fmtStaleAge(cached.at)})</i>`,
        );
        lines.push(`⚠️ <i>${esc(describeProfileError(r.error))}</i>`);
        // newProfilesState already has this entry from the spread above;
        // nothing to do here.
      } else {
        lines.push(`⚠️ <i>${esc(describeProfileError(r.error))}</i>`);
      }
  ...
  if (warn > 0) {
    ...
    if (stale === warn && ok === 0) {
      lines.push(
        `<i>API down — showing cached numbers from earlier. Tap 🔄 to retry.</i>`,
      );
    } else if (stale > 0) {
      lines.push(
        `<i>${ok} live, ${stale} cached. Tap 🔄 to refresh stale profiles.</i>`,
      );
    } else {
      lines.push(
  ...
  // Persist new baseline so the next /status can compute deltas. Save when
  // at least one profile produced numbers (live OR cached) so we never
  // overwrite a useful baseline with zeros, but also don't lose ground when
  // every profile fell back to cache.
  if (ok > 0 || stale > 0) {
    saveBotState({
      ...prev,
      profiles: newProfilesState,
      // Only update the totals snapshot when we got at least one LIVE
      // reading. Cached-only totals would freeze deltas at 0, hiding the
      // actual mining progress that's still happening server-side.
      totals:
        ok > 0
          ? { balance: totalBalance, minted: totalMinted, at: now }
          : prev.totals,
    });
  }
...
// Commands that hit the upstream rpow2 API and may take a few seconds when
// the API is busy. We send a "Fetching ..." placeholder for these so the
// user gets immediate visual feedback instead of staring at a blank screen.
const SLOW_CMDS = new Set(['/status', '/me', '/balance', '/ledger']);
...
async function handleUpdate(token, allow, update) {
  ...
  // Send a placeholder for slow commands so the user sees instant
  // feedback. We'll edit it in place once the real reply is ready.
  let placeholder = null;
  if (SLOW_CMDS.has(parsed.cmd)) {
    placeholder = await tgCall(token, 'sendMessage', {
      chat_id: chatId,
      text: '🔄 <i>Fetching latest data …</i>',
      parse_mode: 'HTML',
      reply_markup: MAIN_KEYBOARD,
    }).catch(() => null);
  }
  ...
  if (placeholder && placeholder.message_id) {
    // Edit the placeholder in place. Inline 🔄 Refresh attaches for
    // status/ledger/profiles; the persistent reply keyboard is unaffected
    // by message-level edits, so the bottom buttons stay visible.
    const inlineMarkup =
      action === 'status' || action === 'ledger' || action === 'profiles'
        ? refreshKeyboard(action)
        : undefined;
    ...
    if (!edited) {
      // Edit failed (e.g., HTML parse error); fall back to plain reply.
