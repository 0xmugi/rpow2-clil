...
// Single /me fetch with retry for transient network errors. Tuned for
// interactive bot responses: ~7s timeout × 2 attempts = worst case ~14s,
// then we fall back to cached data so the user always sees their numbers
// even when api.rpow2.com is having a bad night.
async function fetchMeWithRetry(state, retries = 2, perAttemptTimeoutMs = 7000) {
  let lastErr;
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      const me = await withTimeout(api.me(state), perAttemptTimeoutMs, '/me');
      return { ok: true, me };
    } catch (e) {
      lastErr = e;
      if (
      ...
      // Shorter backoff so we don't pad interactive replies.
  ...
  for (const r of results) {
    ...
    if (r.error) {
      ...
      if (cached && typeof cached.balance === 'number' && typeof cached.at === 'number') {
        ...
        // Keep the same baseline so we don't lose the delta window when
        // the API recovers.
        newProfilesState[r.name] = cached;
      } else {
        lines.push(`⚠️ <i>${esc(describeProfileError(r.error))}</i>`);
      }
      lines.push('');
